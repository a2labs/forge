...
Application.prototype.start = function() {
    if ( ! this.child.process || ! this.child.process.running ) {
        ...
        process.on( 'start', function( err, data ) {
            postal.channel( "forge", "log.start" ).publish( { data: "[forge] Process started with PID " + data.pid } );
            self.child.data = data;
        });

        process.on( 'exit', function( code, signal ) {
            postal.channel( "application", "log.exit" ).publish( { code: code, signal: signal } );
            postal.channel( "forge", "log.exit" ).publish( "[forge] Process has terminated" );
        } );

        process.on( 'stdout', function( data ) {
            postal.channel( "application", "log.output" ).publish( { data: data.toString() } );
        });

        process.on( 'stderr', function( data ) {
            postal.channel( "application", "log.error" ).publish( { data: data.toString() } );
            if (/^execvp\(\)/.test(data)) {
...
Application.prototype.restart = function() {
    var self = this;
    postal.channel( "forge", "log.restart" ).publish( { data: "[forge] Restarting Process"} );
    this.stop( true );
};
