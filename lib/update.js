...
var updater = {
    ...
    gitResetRevision: function( revision, options, done ) {
        options = options || {};
        postal.channel( "forge", "log.output").publish( {data: "[forge] Resetting to revision " + revision});
    ...
    runPostUpdateScript: function( app, options, done ) {
        ...
        if ( app.config.scripts && app.config.scripts.update ) {
            ...
            postal.channel( "forge", "log.output").publish( {data: 'No update script specified' } );
    ...
    fsm: function( app ) {
        ...
        var fsm = new machina.Fsm( {
            ...
            states: {
                ...
                waiting: {
                    "*": function() {
                        console.log("Still waiting to start...");
                    }
                },
